UPDATE prompts SET is_active = 0 WHERE name = 'capability_instructions';

INSERT INTO prompts (name, version, content, description, category, is_active, metadata, created_at, updated_at) 
VALUES (
...
- <capability name="memory" action="search" query="what to search for" /> - Search stored memories
- <capability name="memory" action="recent" limit="5" /> - Get recent memories
...
Instructions:
1. Respond naturally to the user message
...
5. IMPORTANT: When searching memories, use action="search" not action="recall"

User message: {{USER_MESSAGE}}',
  'Fixed capability instructions with correct memory search format',
  'system',
  1,
  '{}',
  datetime('now'),
