...
export class CorrelationContext {
  ...
  /**
   * Get correlation ID for a Slack message
   */
  static getForMessage(messageTs: string): string {
    const existing = this.get(`message:${messageTs}`);
    if (existing) return existing;

    const correlationId = generateCorrelationId();
    this.set(`message:${messageTs}`, correlationId);
    return correlationId;
  }

  /**
   * Get correlation ID for a Slack user session
   */
