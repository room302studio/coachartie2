...
/**
 * Bulletproof path resolver for Slack service
...
export class PathResolver {
  ...
  /**
   * Get path for Slack status file
   */
  public getStatusFilePath(): string {
    return process.env.SLACK_STATUS_FILE || join(this.dataDir, 'slack-status.json');
  }

  /**
   * Get path for Slack metrics file
   */
  public getMetricsFilePath(): string {
    return process.env.SLACK_METRICS_FILE || join(this.dataDir, 'slack-metrics.json');
  }

  /**
   * Get path for Slack events file
   */
  public getEventsFilePath(): string {
    return process.env.SLACK_EVENTS_FILE || join(this.dataDir, 'slack-events.json');
  }
