...
export class SlackTelemetry {
  ...
  // Slack API metrics
  incrementBlocksSent(): void {
    this.metrics.blocksSent++;
  }
  ...
  incrementApiErrors(error: string): void {
    ...
    this.logEvent('slack_api_error', { error }, undefined, undefined, undefined, false);
  }

  incrementReconnections(): void {
    this.metrics.reconnections++;
    this.logEvent('slack_reconnection');
  }
  ...
  // Slack connection metrics
  updateConnectionMetrics(workspaceCount: number, channelCount: number): void {
  ...
  getMetrics(): SlackMetrics & { uniqueUserCount: number } {
    this.updateConnectionMetrics(this.metrics.workspaceCount, this.metrics.channelCount);
  ...
  getHealthSummary(): {
    ...
    if (metrics.apiErrors > 10) {
      issues.push(`Multiple Slack API errors: ${metrics.apiErrors}`);
      status = 'unhealthy';
    }
...
export const telemetry = new SlackTelemetry();
