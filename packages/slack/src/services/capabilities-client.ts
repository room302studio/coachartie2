...
export class CapabilitiesClient {
  ...
  async submitJob(
    ...
    try {
      // AUTO-DETECT: Set source to 'slack' if context contains Slack metadata
      const source = context?.platform === 'slack' ? 'slack' : undefined;
      ...
      const response = await fetch(`${this.baseUrl}/chat`, {
        ...
        body: JSON.stringify({
          ...
          userId,
          context,
          source, // Pass source to trigger Slack-specific handling
        }),
      });
