import { Client, ActionRowBuilder, ButtonBuilder, SelectMenuBuilder, ModalBuilder } from 'discord.js';
...
async function sendButtonMessage(channel: any, userMessage: string, uiData: any): Promise<void> {
  ...
  const actionRows = uiData.actionRows.map((rowData: any) => 
    ActionRowBuilder.from(rowData) as ActionRowBuilder<ButtonBuilder>
  );
