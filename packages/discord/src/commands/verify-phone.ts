...
export const verifyPhoneCommand = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('code').setDescription('The 6-digit verification code').setRequired(true)
    ),
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: '📱 Linked Number',
            value: verification.phoneNumber.replace(/(\+\d{1,3})\d{6}(\d{4})/, '$1******$2'),
            inline: false,
          },
          {
            name: '🔔 Notifications',
            value: 'You can now receive SMS notifications from Coach Artie',
            inline: false,
          },
