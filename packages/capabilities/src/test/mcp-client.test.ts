...
describe('MCP Client Capability', () => {
  it('should be properly configured', () => {
    ...
    expect(mcpClientCapability.description).toBe(
      'Connects to and manages MCP (Model Context Protocol) servers'
    );
    expect(typeof mcpClientCapability.handler).toBe('function');
  });

  it('should throw error for unknown action', async () => {
    await expect(mcpClientCapability.handler({ action: 'unknown' })).rejects.toThrow(
      'Unknown MCP client action: unknown'
    );
  });

  it('should require URL for connect action', async () => {
    await expect(mcpClientCapability.handler({ action: 'connect' })).rejects.toThrow(
      'MCP server URL is required'
    );
  });

  it('should require connection ID for disconnect action', async () => {
    await expect(mcpClientCapability.handler({ action: 'disconnect' })).rejects.toThrow(
      'Connection ID is required'
    );
  });

  it('should require connection ID for call_tool action', async () => {
    await expect(mcpClientCapability.handler({ action: 'call_tool' })).rejects.toThrow(
      'Connection ID is required'
    );
  });
