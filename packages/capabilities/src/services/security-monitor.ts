...
interface SecurityIncident {
  ...
  userId: string;
  messageId: string;
  incidentType: 'structured_output_leak' | 'debug_info_leak' | 'system_prompt_leak' | 'internal_reasoning_leak';
  patterns: string[];
...
export class SecurityMonitor {
  ...
  logSanitizationEvent(
    ...
    const reductionPercent = Math.round(((originalContent.length - sanitizedContent.length) / originalContent.length) * 100);
  ...
  private analyzeSecurityPatterns(content: string): string[] {
    ...
    if (content.match(/^analysis[A-Z]/im)) patterns.push('analysis_leak');
    if (content.match(/^assistant\s*commentary/im)) patterns.push('assistant_commentary');
    if (content.match(/^assistant\s*final/im)) patterns.push('assistant_final_marker');
    if (content.match(/<thinking>/i)) patterns.push('thinking_tags');
    
    // Debug and system patterns  
    if (content.match(/^(DEBUG|TRACE|INFO):/im)) patterns.push('debug_output');
    if (content.match(/^\[SYSTEM/im)) patterns.push('system_messages');
    if (content.match(/^(execute|processing|result):/im)) patterns.push('execution_debug');
    
    // Structured data patterns
    if (content.match(/json\s*\{/i)) patterns.push('json_leak');
    if (content.match(/^→\s*/m)) patterns.push('arrow_fragments');
    if (content.match(/^:\s*/m)) patterns.push('colon_fragments');
    
    // XML debug patterns
    if (content.match(/<(analysis|commentary|debug|internal)/i)) patterns.push('xml_debug_tags');
  ...
  private categorizeIncident(patterns: string[]): SecurityIncident['incidentType'] {
    ...
    if (patterns.some(p => ['analysis_leak', 'thinking_tags', 'assistant_commentary'].includes(p))) {
   ...
   */
  private calculateSeverity(patterns: string[], reductionPercent: number): SecurityIncident['severity'] {
  ...
  private logIncident(incident: SecurityIncident, contentPreview: string): void {
    const logData = {
      ...
      contentPreview: contentPreview + (contentPreview.length < incident.originalLength ? '...' : '')
    };
  ...
  checkSecurityTrends(): {
    ...
    if (criticalIncidents > 0) {
      recommendations.push('URGENT: Review model configuration - critical information leaks detected');
    }
    if (highIncidentsRecent > 10) {
      recommendations.push('Consider stricter model prompting to reduce internal reasoning exposure');
    }
