...
export class OpenAIResearchHarness implements ModelHarness {
  ...
  type = 'async' as const;
  description =
    'OpenAI o4-mini-deep-research for complex research tasks with web search and code execution';
  ...
  async pollTask(handle: TaskHandle): Promise<TaskStatus> {
    ...
    try {
      ...
      logger.debug(
        `🔬 Task ${handle.id} status: ${status.status} (${status.toolCalls} tool calls)`
      );
      return status;
    } catch (error) {
  ...
  async getResult(handle: TaskHandle): Promise<TaskResult> {
    ...
    try {
      ...
      const codeExecutions =
        data.tool_calls?.filter((t) => t.type === 'code_interpreter').length || 0;

      const cost = {
        tokens:
          ((data.usage?.input_tokens || 0) * 2 + (data.usage?.output_tokens || 0) * 8) / 1_000_000,
  ...
  estimateCost(task: ResearchTask): CostEstimate {
    ...
    return {
      min: 0.3, // Just tokens, minimal tools
      max: 3.0, // Heavy research, many searches
      typical: hasWebSearch && hasCode ? 1.1 : 0.5,
      currency: 'USD',
    };
  }
