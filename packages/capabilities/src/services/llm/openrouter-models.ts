...
export class OpenRouterModelsService {
  ...
  async getEnhancedModelData(activeModels: string[], currentModel: string): Promise<any[]> {
    ...
    return activeModels.map((modelId) => {
      ...
      return {
        ...
        // Technical specs
        contextLength:
          openrouterInfo?.context_length ||
          openrouterInfo?.top_provider?.context_length ||
          'Unknown',
  ...
  async getModelSummary(activeModels: string[]): Promise<{
    ...
    const paidModelsWithCost = enhancedData.filter((m) => !m.isFree && m.inputCostPer1K !== 'N/A');

    const averageInputCost =
      paidModelsWithCost.length > 0
