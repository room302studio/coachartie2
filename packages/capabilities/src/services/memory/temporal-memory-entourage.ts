...
export class TemporalMemoryEntourage implements MemoryEntourageInterface {
  ...
  async getMemoryContext(
    ...
    try {
      ...
      const formattedContent = this.formatTemporalMemories(
        temporalMatches,
        userMessage,
        options.maxTokens
      );
      ...
      logger.info(
        `⏰ TemporalMemoryEntourage found ${temporalMatches.length} temporal patterns (confidence: ${confidence.toFixed(2)})`
      );
  ...
  private analyzeTemporalRelevance(
    ...
    const hourDiff = Math.min(
      Math.abs(currentHour - memoryHour),
      24 - Math.abs(currentHour - memoryHour)
    );
  ...
  private matchTemporalContext(
    ...
    for (const keyword of temporalKeywords) {
      ...
      if (keyword === 'recently' && this.isRecent(memoryDate, currentDate, 7)) {
        score += 0.2;
      }
      if (keyword === 'yesterday' && this.isDaysAgo(memoryDate, currentDate, 1)) {
        score += 0.3;
      }
      if (keyword === 'today' && this.isToday(memoryDate, currentDate)) {
        score += 0.4;
      }
      if (keyword === 'weekend' && this.isWeekend(memoryDate)) {
        score += 0.2;
      }
      if (keyword === 'morning' && this.isMorning(memoryDate)) {
        score += 0.15;
      }
      if (keyword === 'evening' && this.isEvening(memoryDate)) {
        score += 0.15;
      }
    }
  ...
  private formatTemporalMemories(
    ...
    const styles = ['temporal_contextual', 'temporal_chronological', 'temporal_pattern'];
  ...
  private formatRelativeTime(dateString: string): string {
    ...
    if (daysDelta < 7) {
      return `${daysDelta} days ago`;
    }
    if (daysDelta < 30) {
      return `${Math.floor(daysDelta / 7)} weeks ago`;
    }
    if (daysDelta < 365) {
      return `${Math.floor(daysDelta / 30)} months ago`;
    }
  ...
  private detectTemporalPatterns(memories: Array<{ temporalType: string }>): string {
    ...
    const dominantType = Array.from(typeCount.entries()).sort(([, a], [, b]) => b - a)[0]?.[0];

    switch (dominantType) {
      case 'recent':
        return 'From recent patterns';
      case 'historical':
        return 'From historical patterns';
      case 'time_of_day':
        return 'From similar times';
      case 'day_of_week':
        return 'From similar days';
      default:
        return 'From temporal patterns';
    }
  }
