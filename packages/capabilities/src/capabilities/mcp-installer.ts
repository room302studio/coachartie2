...
💡 Try using one of the suggested templates above.`);
    ...
    throw new Error(
      `❌ Unknown MCP template: "${templateName}". Available templates: ${availableTemplates.join(', ')}`
    );
  }
  ...
  for (const packageName of packagesToTry) {
    ...
    try {
      ...
      try {
        await capabilityRegistry.execute('filesystem', 'delete', {
          path: mcpPath,
          recursive: true,
        });
  ...
  // All packages failed
  const errorReport = generateDetailedErrorReport(
    templateName,
    packagesToTry,
    lastError,
    attemptCount
...
async function installMCPPackage(
  ...
  try {
    ...
    const config = {
      ...
      environment: template.envVars.reduce(
        (acc: Record<string, string>, envVar: string) => {
          acc[envVar] = `\${${envVar}}`;
          return acc;
        },
    ...
    if (template.envVars.length > 0) {
      ...
      const envContent = template.envVars
        .map((envVar: string) => {
...
4. Use the 'start_mcp_server' action to start the server`
    : '🚀 No environment setup required - you can start the server immediately!'
}

💡 Use 'start_mcp_server' action with name="${templateName}" to start the server.`;
  } catch (error) {
    throw new Error(
      `Failed to install ${templateName} MCP: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function createCustomMCP(name: string, description?: string, port?: number): Promise<string> {
  ...
  try {
    ...
    await capabilityRegistry.execute('filesystem', 'create_directory', { path: mcpPath });
    await capabilityRegistry.execute('filesystem', 'create_directory', {
      path: join(mcpPath, 'src'),
    });
...
💡 Use 'start_mcp_server' action with name="${name}" to start the server.`;
    ...
    throw new Error(
      `Failed to create custom MCP server ${name}: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
✨ The MCP server is now ready to start with proper environment configuration.`;
    ...
    throw new Error(
      `Failed to setup environment for ${mcpName}: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function startMCPServer(mcpName: string): Promise<string> {
  ...
  try {
    ...
    if (!mcpExists.includes('directory exists')) {
      ...
      const customExists = await capabilityRegistry.execute('filesystem', 'exists', {
        path: customMcpPath,
      });
      ...
      throw new Error(
        `MCP server '${mcpName}' not found. Available templates: ${Object.keys(MCP_TEMPLATES).join(', ')}`
      );
    }

    // Check if there's a startup script
    const startupScriptPath = join(mcpPath, 'start.sh');
    const scriptExists = await capabilityRegistry.execute('filesystem', 'exists', {
...
💡 Use 'check_mcp_status' to monitor the server status.`);
    ...
    throw new Error(
      `Failed to start MCP server ${mcpName}: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function startCustomMCPServer(mcpName: string, mcpPath: string): Promise<string> {
  ...
  try {
    // Check if it's built
    const distExists = await capabilityRegistry.execute('filesystem', 'exists', {
      path: join(mcpPath, 'dist'),
    });
...
💡 Use 'check_mcp_status' to monitor the server status.`);
    ...
    throw new Error(
      `Failed to start custom MCP server ${mcpName}: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function checkMCPStatus(): Promise<string> {
  ...
  try {
    ...
    const mcpDirExists = await capabilityRegistry.execute('filesystem', 'exists', {
      path: mcpServersPath,
    });
...
💡 Use 'create_custom_mcp' to create a custom MCP server.`;
    ...
    const mcpDirListing = await capabilityRegistry.execute('filesystem', 'list_directory', {
      path: mcpServersPath,
    });
...
💡 Use 'create_custom_mcp' to create a custom MCP server.`;
    ...
    for (const entry of entries) {
      ...
      if (serverName === 'custom') {
        ...
        const customListing = await capabilityRegistry.execute('filesystem', 'list_directory', {
          path: customPath,
        });

        if (!customListing.includes('is empty')) {
          const customEntries = customListing
            .split('\n')
            .filter((line) => line.trim().startsWith('📁'));
        ...
        // Try to read config
        try {
          const configExists = await capabilityRegistry.execute('filesystem', 'exists', {
            path: configPath,
          });
          if (configExists.includes('file exists')) {
            const configContent = await capabilityRegistry.execute('filesystem', 'read_file', {
              path: configPath,
            });
    ...
    statusReport.push(
      '   💡 Use system process monitoring tools to check if MCP servers are running'
    );
    ...
    statusReport.push(
      '   • Use "setup_environment" to configure API keys and environment variables'
    );
    ...
    throw new Error(
      `Failed to check MCP status: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
function getEnvVarDescription(envVar: string): string {
  const descriptions: Record<string, string> = {
    ...
    POSTGRES_CONNECTION_STRING:
      'PostgreSQL database connection string (postgresql://user:pass@host:port/db)',
  };
...
function calculateStringSimilarity(a: string, b: string): number {
  ...
  for (const aWord of aWords) {
    for (const bWord of bWords) {
      if (aWord === bWord) {
        wordMatches++;
      } else if (aWord.includes(bWord) || bWord.includes(aWord)) {
        wordMatches += 0.5;
      }
    }
  }
 ...
 */
function analyzeInstallationError(
  error: Error,
  packageName: string
  ...
  if (errorMessage.includes('version') || errorMessage.includes('incompatible')) {
    return {
      ...
      suggestion:
        'Version conflict detected. Try updating dependencies or using a different version.',
    };
  }
...
function generateDetailedErrorReport(
  ...
  if (lastError) {
    const analysis = analyzeInstallationError(
      lastError,
      packagesAttempted[packagesAttempted.length - 1]
    );
  ...
  if (templateName.includes('weather')) {
    ...
    weatherTemplates.forEach((template) => {
      if (template !== templateName) {
        report.push(
          `     - ${template}: ${MCP_TEMPLATES[template as MCPTemplateName].description}`
        );
      }
    });
  }
  ...
  report.push(
    '     `<capability name="mcp_installer" action="create_custom_mcp" name="my_weather_server" />`'
  );
 ...
 */
async function handleMCPInstallerAction(
  params: MCPInstallerParams,
  content?: string
): Promise<string> {
  const { action } = params;
 ...
 */
async function handleInstallFromTemplate(
  params: MCPInstallerParams,
  _content?: string
): Promise<string> {
  ...
  if (!templateName) {
    throw new Error(
      `Template name is required. Available templates: ${Object.keys(MCP_TEMPLATES).join(', ')}`
    );
  }
...
async function handleInstallFromGitHub(
  ...
  if (!isValidGitHubUrl(githubUrl)) {
    throw new Error(
      `Invalid GitHub URL: ${githubUrl}. Expected format: https://github.com/user/repo`
    );
  }
 ...
 */
async function handleCreateCustomMCP(
  params: MCPInstallerParams,
  content?: string
): Promise<string> {
 ...
 */
async function handleCheckMCPStatus(
  _params: MCPInstallerParams,
  _content?: string
): Promise<string> {
...
The server is ready to use. You can start it with the 'start_mcp_server' action using name "${repo}".`;
    ...
    throw new Error(
      `Failed to install MCP server from GitHub: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function detectMCPServer(serverPath: string): Promise<{
  ...
  if (await fileExists(readmePath)) {
    try {
      ...
      if (
        mcpIndicators.some((indicator) => readme.toLowerCase().includes(indicator.toLowerCase()))
      ) {
        return {
          isMCP: true,
  ...
  const mcpFiles = ['mcp.json', 'mcp-config.json', 'tools.json', 'src/tools.ts', 'src/server.ts'];
...
export const mcpInstallerCapability: RegisteredCapability = {
  ...
  description:
    'Autonomous MCP (Model Context Protocol) installation and management capability that orchestrates filesystem, package management, and environment setup',
