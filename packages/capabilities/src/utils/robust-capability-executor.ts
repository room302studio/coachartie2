...
export class RobustCapabilityExecutor {
  ...
  async executeWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        logger.info(`🔄 Attempt ${attempt}/${maxRetries} for ${capability.name}:${capability.action}`);
        ...
        logger.warn(`❌ Attempt ${attempt} failed for ${capability.name}:${capability.action}: ${lastError.message}`);
    ...
    logger.warn(`🚨 All retries failed for ${capability.name}:${capability.action}, trying fallbacks`);
  ...
  private cleanCapability(capability: ParsedCapability, attempt: number): ParsedCapability {
    ...
    logger.info(`🧹 CLEANED: ${capability.name}:${capability.action} - params: ${JSON.stringify(cleaned.params)}`);
    return cleaned;
  }
  ...
  private async tryFallbackExecution(
    ...
    switch (capability.name) {
      case 'calculator':
        return this.fallbackCalculation(capability.content || String(capability.params.expression || ''));
  ...
  private getHelpfulErrorMessage(capability: ParsedCapability, error: Error | null): string {
    ...
    const suggestion = suggestions[baseName as keyof typeof suggestions] || 'Please try again later';
