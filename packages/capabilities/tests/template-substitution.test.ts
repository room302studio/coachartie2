...
class TemplateSubstitution {
   ...
   */
  static createVariablesFromResults(
    results: Array<{ capability: { name: string }; data: unknown }>
...
describe('Template Variable Substitution', () => {
  ...
  it('should handle LEGO-block orchestration pattern', () => {
    ...
    const results = [
      {
        ...
        data: 'User achievements: Led 3 major projects, Expert in TypeScript',
      },
    ];

    const variables = TemplateSubstitution.createVariablesFromResults(results);

    const webPrompt = TemplateSubstitution.substitute('Format as resume: {{memories}}', variables);

    expect(webPrompt).toBe(
      'Format as resume: User achievements: Led 3 major projects, Expert in TypeScript'
    );
    ...
    const filesystemContent = TemplateSubstitution.substitute('{{content}}', updatedVariables);

    expect(filesystemContent).toBe(
      '# Resume\n\n## Experience\n- Led 3 major projects\n- Expert in TypeScript'
    );
  });
  ...
  it('should handle complex nested templates', () => {
    ...
    expect(result).toBe(
      'Previous: chocolate preferences, Current: final output, Memory: chocolate preferences'
    );
  });
});
